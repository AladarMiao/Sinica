...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        procedureHelp = scheme_eval(first, env)
        check_procedure(procedureHelp)
        return Procedure.eval_call(procedureHelp, rest, env)
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('bruh the number of parameters aint right to me')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        proc = expressions.second
        env.define(target.first, LambdaProcedure(formals, proc, env))
        return target.first
        # END PROBLEM 10
    else:
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            procedureHelp = scheme_eval(first, env)
            check_procedure(procedureHelp)
            result = Procedure.eval_call(procedureHelp, rest, env)
