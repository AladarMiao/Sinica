...
'(1 2)
; expect (1 2)
(car (quote (pranav vachaspati)))
; expect pranav
(cdr (quote (pranav vachaspati)))
; expect (vachaspati)
(quote (1 2 3))
; expect (1 2 3)
(define formula (lambda x y z: (y+sqrt(y*y-4*x*z))/2))
; expect formula
(formula 1 2 1)
; expect Error
(define nani (lambda (x) (* x 34)))
; expect nani
(nani 5)
; expect 170
(define (fib n)
...
(and True (1/0) False):
; expect Error
(cond ((< 3.14 1.414) 1.732)
      ((< 1.618 2.414) 3.1415926)
      )
; expect 3.1415926
(define nautilus 1.6)
; expect nautilus
(let ((nautilus 3)
      (nautilusNo2 (+ nautilus 2)))
  (* 1 nautilusNo2))
; expect 3.6
(+ 1 2)
; expect 3
...
; expect (did you ever hear the tragedy of darth plagueis the wise)
'(1 (1 2 . (3 . 5)))
; expect (1 (1 2 3 . 5))
(odd? 13579)
; expect True
...
(and True 1 2 3)
; expect 3
(if (= (fib 3) (fib 8)) true false)
; expect False
(cond ((= (+ 2 2) (- 4 1)) 'quickmaths)
      ((and 1 2 3 False) 'test)
      ((= 42 42) 'correct))
; expect correct
(let ((a 1) (b 1))
  (let ((c (* a b)))
    c))
; expect 1
(define anakin (mu (x) (/ x y)))
(define vader (lambda (x y) (anakin (+ x y))))
(vader 10 20)
; expect 1.5
;;; **********************************
